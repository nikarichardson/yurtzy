...
## Website
The `backend site` is hosted through Heroku at <a href="https://yurtzy.herokuapp.com/">yurtzy.herokuapp.com</a>. Essentially, the `backend site`</a> only returns JSON objects for each endpoint. For more views of a possible frontend implementation of the site, you can also look at the `Screenshots` folder. 
